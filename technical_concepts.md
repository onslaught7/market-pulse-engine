# MarketPulse Engine: Technical Deep Dive & Architectural Walkthrough

> **Repo Name:** `market-pulse-engine` (formerly `vortex-rag-engine`)
> **Role:** Senior Backend Engineer & Tech Tutor
> **Goal:** To explain the architecture so you can discuss it confidently.
...
* **The Asynchronous Way (MarketPulse):** You hire a Doorman (Go Gateway) and a Chef (Python Worker). The Doorman just greets people and gives them a ticket (Task ID). The Chef cooks in the background. The line moves instantly.
...
## 4. The Data Strategy: Metadata Tagging (The "Financial Passport")

This is what elevates **MarketPulse** from a generic RAG system to a Financial Intelligence Engine. We do not just dump text into the database; we stamp every document with a "Metadata Passport."

### 4.1. The Problem: "Global Confusion"

A standard RAG system treats all text equally. If a user asks "What is the capital gains tax?", a standard system might retrieve a paragraph from a **US 10-K filing** and a paragraph from an **Indian SEBI report**, mashing them together into a confusing, wrong answer.

### 4.2. The Solution: Structured Context

We attach a JSON payload to every vector stored in Qdrant. This allows us to perform **Filtered Search** (e.g., "Only look for tax laws where `region` is `India`").

### 4.3. The Implementation (The Schema)

Every document ingested goes through a classification mapping before storage:

```python
# The Metadata Map (Logic Layer)
LIBRARY_MAP = {
    "stocks_to_riches.pdf": {
        "title": "Stocks to Riches",
        "region": "india",       # <--- The Passport Stamp
        "topic": "growth_investing"
    },
    "intelligent_investor.pdf": {
        "title": "The Intelligent Investor",
        "region": "global",      # <--- Applies to all markets
...
### 4.4. The Vector Payload (Storage Layer)

When this data hits Qdrant, a single "point" (paragraph) looks like this:
...
```json
{
  ...
  "vector": [0.12, -0.98, ...],
  "payload": {
    "text": "In Indian markets, volatility is often...",
    "source_type": "book",
    "region": "india",
    "topic": "growth_investing",
    "author": "Parag Parikh"
...
**Why this matters:**
This enables **"Surgical Retrieval."**

* **User Query:** "How to pick stocks in Mumbai?"
* **System Logic:** `filter: { should: [ { key: "region", match: "india" }, { key: "region", match: "global" } ] }`
* **Result:** The system explicitly *excludes* US-specific advice, ensuring high-accuracy responses.

---

## 5. Engineering "Buzzwords" Meaning

* **Throughput vs. Latency:**
* **Throughput:** How many cars can pass through a toll booth per hour. (MarketPulse optimizes for this).
* **Latency:** How long it takes one car to get from A to B. (MarketPulse might actually have *slightly* higher latency per request because of the Redis hop, but it prevents the system from crashing, which is worth it).
...
* In MarketPulse, the orders just pile up in the Ticket Rail (Redis). The chefs keep working at their own pace. The waiters keep seating guests. The system bends, but it doesn't break. This is handling **Backpressure**.
...
## 6. Summary for your Conversation
...
* **MarketPulse Approach:** The "Modern Polyglot" solution. Using Go for the gateway eliminates the overhead of Python's web server concurrency issues (GIL), while keeping Python for what it's good at (AI).
...
## 9. Go for Python Developers: A Complete Syntax Deep-Dive
...
### 9.1. The Foundation: `package main` (Line 1)
...
### 9.2. Importing Dependencies: The `import` Block (Lines 3-11)
...
### 9.3. Defining Data Shapes: `type` and `struct` (Lines 14-19)
...
### 9.4. Package-Level Variables: `var` (Line 21)
...
### 9.5. The Entry Point: `func main()` (Line 23)
...
### 9.6. Short Variable Declaration: `:=` (Lines 24-27)
...
### 9.7. Creating a Redis Client: `&` and Options (Lines 29-31)
...
### 9.8. Testing the Connection: Method Chaining and Error Handling (Lines 33-37)
...
### 9.9. Creating the Fiber App (Line 39)
...
### 9.10. Defining a Route Handler: Anonymous Functions (Lines 41-60)
...
### 9.11. Starting the Server (Line 62)
...
### 9.12. Summary: Go Syntax Quick Reference
...
## 10. Code Deep Dive: `gateway/Dockerfile` (Multi-Stage Builds)
...
## 11. Code Deep Dive: `worker/worker.py` (The Python Consumer)
...
### 11.2. Connecting to Redis (Lines 6-11)
...
### 11.4. The Worker Loop: BRPOP (Lines 28-45)
...
### 11.5. The Entry Point (Lines 47-50)
...
## 12. Code Deep Dive: `worker/Dockerfile` (Modern Python Packaging)
